...
to get-pollution ;; computes the number of CO2 molecules required.
  ...
  let f1 fval Walk_to_school ;; sets f1 to 0, 1, 2, 3, 4, depending on the user input
  let f2 fval Turn_off_lights
   set car-pollution  8 * f1 ;; non-walking pollutes a lot
  set truck-pollution f1 + 0 * f2 + 0 ;; walking, heat/cool, veg, and showers
  set factory-pollution f1 + 2 * f2 + 0  ;; walking, Al, heat/cool, elect, veg, showers
end
